...
public class Menu_Window extends JFrame implements ActionListener {
     ...
     */
    public Menu_Window() {
        System.out.println("Defoult constructor from Menu_Window called (probably by WindowBuilder)");
    ...
    private ImageIcon loadIcon(String path) {
        try {
            ...
            if (imgURL != null) {
                ...
                System.err.println("Error: the image resource was not found: " + path);
                return null; // Devolver null si no se encuentra
            }
        } catch (Exception e) {
            System.err.println("Error when loading icono: " + path + " - " + e.getMessage());
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source == btnGame_1_aim) {
            // Abrir ventana del juego (sin modificar puntos aquí)
             System.out.println("Opening Aim Game for: " + j.getName());
            ...
            System.out.println("Opening Math Game for: " + j.getName());
           ...
           // --- !!! FIN CORRECCIÓN !!! ---
            ...
            try {
                ...
                System.out.println( j.getPoints() + "'s points were updated before opening Trophy_Window.");
            } catch (Exception ex) {
                System.err.println("An error occurred when obtaining the Score: " + ex.getMessage());
                 JOptionPane.showMessageDialog(this, "Unable to update the points.\nThe last value will be updated.", "Warning", JOptionPane.WARNING_MESSAGE);
            }
            // --- Fin actualización ---
            ...
            // trophyWindow.setVisible(true); // Es modal, setVisible es llamado por el constructor implícitamente? No, hay que llamarlo.
             ...
             System.out.println("Opening Stats for: " + j.getName());
            ...
            int option = JOptionPane.showConfirmDialog(
                ...
                "Confirm The DELETEING of your account",
                JOptionPane.YES_NO_OPTION,
                JOptionPane.WARNING_MESSAGE);

            if (option == JOptionPane.YES_OPTION) {
                 try {
                     System.out.println("Deleteing the account of: " + j.getName());
                    cont.deletePlayer(j);
                    JOptionPane.showMessageDialog(this, "Account succesfully deleted", "Deletion complete", JOptionPane.INFORMATION_MESSAGE);
                    // Volver a Login
                    Login_Window loginWindow = new Login_Window(cont);
                    loginWindow.setVisible(true);
                    this.dispose(); // Cerrar ventana de Menú
                 } catch (Exception ex) {
                     System.err.println("An error occurred when delteing the account: " + ex.getMessage());
                     ex.printStackTrace(); // Para depuración
                     JOptionPane.showMessageDialog(this, "An error occurred when delteing the account.", "Error", JOptionPane.ERROR_MESSAGE);
                 }
            }
            ...
            if (option == JOptionPane.YES_OPTION) {
                 System.out.println("Logging  " + j.getName() +" out");
                Login_Window loginWindow = new Login_Window(cont);
                loginWindow.setVisible(true);
                this.dispose(); // Cerrar ventana de Menú
            }
