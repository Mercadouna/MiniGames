    ...
    private void endGame() {
        ...
        if (cont != null && player != null && score >= 0) { // Guardar si score >= 0
             try {
                 ...
                 System.out.println("Trying to save game: Player=" + player.getName() + ", Game=" + gameName + ", Score=" + score);
                 cont.savePlayResult(this.player, gameName, this.score); // Guarda en PLAYS
             } catch (Exception ex) {
                  System.err.println("Error calling savePlayResult: " + ex.getMessage());
        ...
        if (cont != null && player != null && score > 0) { // Solo añadir si el score es positivo
             try {
                 System.out.println("Adding " + score + " points to the player " + player.getName());
                 cont.addPoints(player, score); 
                 player.setPoints(player.getPoints() + score);
                 System.out.println("Local points updated to: " + player.getPoints());

             } catch (Exception ex) {
                 System.err.println("Error when adding final score: " + ex.getMessage());
                 // Mostrar un error al usuario sería útil aquí
                 JOptionPane.showMessageDialog(this,
                       "An error ocurred when saving Score.",
                       "Error when trying to save scre", JOptionPane.ERROR_MESSAGE);
             }
        }
